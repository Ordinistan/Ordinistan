...
exports.redeemHTLC = redeemHTLC;
exports.refundHTLC = refundHTLC;
...
function createHTLC(options) {
    ...
    if (!swapParams.contractHash) {
        swapParams.contractHash = crypto
            .createHash('sha256')
            .update(Buffer.from(swapParams.preimage, 'hex'))
            .digest('hex');
    }
    const script = getWitnessScript(swapParams.recipientAddress, swapParams.refundAddress, swapParams.contractHash, swapParams.expiration);
    const p2wsh = bitcoin.payments.p2wsh({
        redeem: { output: script, network },
        network
    });
    const fullSwapParams = swapParams;
...
function redeemHTLC(options) {
    const { preimage, recipientWIF, witnessScript, txHash, value, feeRate, vout } = options;
...
function refundHTLC(options) {
    const { refundWIF, witnessScript, txHash, value, feeRate, vout } = options;
    ...
    tx.addOutput(bitcoin.address.toOutputScript(bitcoin.payments.p2wpkh({
        pubkey: keyPair.publicKey,
        network: bitcoin.networks.bitcoin,
    }).address, bitcoin.networks.bitcoin), outputAmount);
    // We don't set locktime - defaults to 0
