    ...
    UUPSUpgradeable
{
    ...
    /// @dev Required by UUPS module - Allows only owner to upgrade implementation
    ...
    /// @dev Calculates and transfers royalties if applicable
    /// @return The remaining amount after royalties
    function _handleRoyalties(address nftContract, uint256 tokenId, uint256 salePrice, address paymentToken)
        internal
        returns (uint256)
    {
        try IERC2981(nftContract).royaltyInfo(tokenId, salePrice) returns (address receiver, uint256 royaltyAmount) {
            if (receiver != address(0) && royaltyAmount > 0) {
                if (paymentToken == address(0)) {
                    ...
                    safeTransferAmount(paymentToken, receiver, royaltyAmount);
                }
                return salePrice - royaltyAmount;
            }
        } catch {}
        return salePrice;
    }
    ...
    function buyNow(
        ...
        // First handle the payment transfer
        if (isNative) {
        ...
        uint256 remainingAmount = _handleRoyalties(
            _order.nftContract, 
            _order.tokenId, 
        ...
        // Finally transfer remaining amount to seller
        if (isNative) {
            (bool success,) = payable(_order.seller).call{value: remainingAmount}("");
            require(success, "Transfer Failed");
        } else {
            IERC20Upgradeable ERC20Interface = IERC20Upgradeable(_order.paymentToken);
            ERC20Interface.safeTransfer(_order.seller, remainingAmount);
        }
    ...
    function acceptBid(uint256 orderId, uint256 bidId) external nonReentrant {
        ...
        uint256 remainingAmount =
            _handleRoyalties(_order.nftContract, _order.tokenId, totalAmount - feeValue, _order.paymentToken);
        ...
        if (isNative) {
            ...
            safeTransferAmount(_order.paymentToken, _order.seller, remainingAmount);
        }
    ...
    /// @dev Safe transfer of ERC20 tokens
    function safeTransferAmount(address token, address to, uint256 amount) internal {
